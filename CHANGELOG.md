# Contract Entitlement changelog
...
- Base URL is `https://contractentitlement-api.com`. A client generated from an earlier copy of this contract points elsewhere and must be regenerated.
...
- Plans published. Billed per contract.

| Plan | Per month | Included contracts | Additional | Overage cap |
| --- | --- | --- | --- | --- |
| Developer | $499 | 250 | $3.00 each | $1,000 |
| Growth | $1,499 | 1,000 | $2.25 each | $3,000 |
| Scale | $4,999 | 5,000 | $1.50 each | $10,000 |
| Enterprise | $10,000 | 15,000 | $1.00 each | $20,000 |
...
- First published contract, 9 operations:
  ...
  - `POST /v1/contracts` — Normalize contracts into entitlements, obligations and schedules
  - `POST /v1/entitlements/check` — Ask whether a customer is entitled to a SKU, right now
  - `POST /v1/demo/normalize` — Public demo — normalize one contract without a key (no API key required)
  - `GET /v1/obligation-types` — Every obligation kind and reason code the engine emits (no API key required)
...
- Free sandbox keys allow 25 contracts per period, no card required.
